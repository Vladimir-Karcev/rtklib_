...
//const double chisqr[] = { /* chi-sqr(n) (alpha=0.05) */
...
const prcopt_t prcopt_default = { /* defaults processing options */
	PMODE_SINGLE,0,2,SYS_GPS | SYS_CMP | SYS_CMP,   /* mode,soltype,nf,navsys */
	15.0 * D2R,{{0,0}},           /* elmin,snrmask */
	0,1,1,1,                    /* sateph,modear,glomodear,bdsmodear */
	5,0,10,1,                   /* maxout,minlock,minfix,armaxiter */
	5,1,0,0,                    /* estion,esttrop,dynamics,tidecorr */
	...
	double* xp, double* Pp)
{
	...
	trace(15, "K=\n"); tracemat(5, K, n, m, 15, 5);
	trace(15, "P=\n"); tracemat(5, Pp, n, n, 15, 5);
	trace(15, "Pp=\n"); tracemat(5, Pp, n, n, 15, 5);
	trace(15, "xp=\n"); tracemat(5, xp, n, 1, 15, 5);
	...
	const double* azel)
{
	...
	double r_ion_delay = CLIGHT * f * (fabs(x) < 1.57 ? 5E-9 + amp * (1.0 + x * x * (-0.5 + x * x / 24.0)) : 5E-9);
	return r_ion_delay;
}
...
*
* args   : pos         IN    DOUBLE     Station geodetic coordinates (radians)
*          height      IN    DOUBLE     Station orthometric height (m)
*          doy         IN    DOUBLE     Day of year
*          azel        IN    DOUBLE     Elevation angle (radians)
*          hzd         OUT   DOUBLE     Hydrostatic zenith delay (m)
*          wzd         OUT   DOUBLE     Non-hyd. zenith delay (m)
*
* return :
*-----------------------------------------------------------------------------*/
extern void unb3tropmodel(const double* pos, const double* azel, double height, double doy, double elev, double* hzd, double* wzd) {
	...
	if (!(pos[2] < -100.0 || 1E4 < pos[2] || azel[1] <= 0)) {
		...
		*wzd = 0.0;
		...
		/* Deal with southern hemisphereand yearly variation */
		double tdoy = doy;
		if (latdeg < 0)
			tdoy += 182.625;
		double COSPHS = cos((tdoy - 28) * DOY2RAD);

		/*Initialize pointers to lookup table*/
		double latabs = abs(latdeg);
		int p1 = 0, p2 = 0, m = 0;
		if (latabs >= 75.0) {
			p1 = 5;
			p2 = 5;
			...
			p1 = trunc((latabs - 15) / 15) + 1;
			p2 = p1 + 1;
			int col = 0; // lat column in AVG array
			m = (latabs - AVG[col + p1 * 6]) / (AVG[col + p2 * 6] - AVG[col + p1 * 6]);
		}

		/* Compute average surface tropo values by interpolation*/
		double PAVG = m * (AVG[1 + p2 * 6] - AVG[1 + p1 * 6]) + AVG[1 + p1 * 6];
		double TAVG = m * (AVG[2 + p2 * 6] - AVG[2 + p1 * 6]) + AVG[2 + p1 * 6];
		double EAVG = m * (AVG[3 + p2 * 6] - AVG[3 + p1 * 6]) + AVG[3 + p1 * 6];
		double BETAAVG = m * (AVG[4 + p2 * 6] - AVG[4 + p1 * 6]) + AVG[4 + p1 * 6];
		double LAMBDAAVG = m * (AVG[5 + p2 * 6] - AVG[5 + p1 * 6]) + AVG[5 + p1 * 6];

		/*Compute variation of average surface tropo values*/
		double PAMP = m * (AMP[1 + p2 * 6] - AMP[1 + p1 * 6]) + AMP[1 + p1 * 6];
		double TAMP = m * (AMP[2 + p2 * 6] - AMP[2 + p1 * 6]) + AMP[2 + p1 * 6];
		double EAMP = m * (AMP[3 + p2 * 6] - AMP[3 + p1 * 6]) + AMP[3 + p1 * 6];
		double BETAAMP = m * (AMP[4 + p2 * 6] - AMP[4 + p1 * 6]) + AMP[4 + p1 * 6];
		double LAMBDAAMP = m * (AMP[5 + p2 * 6] - AMP[5 + p1 * 6]) + AMP[5 + p1 * 6];
		...
		double ES = 0.01 * exp(1.2378847e-5 * (T0 * T0) - 1.9121316e-2 * T0 + 3.393711047e1 - 6.3431645e3 * (1.0 / T0));
		...
		double GEOLAT = atan((1.0 - EXCEN2) * tan(pos[0]));
		double DGREF = 1.0 - 2.66e-03 * cos(2.0 * GEOLAT) - 2.8e-07 * height;
...
/* interpolate antenna phase center variation GAMP --------------------------------*/
...
extern void antmodel_gamp(int sat, const pcv_t* pcv, const double* del, const double* azel,
	int opt, double* dant)
{
	double e[3], off[3], cosel = cos(azel[1]);
	int i, j, ii = 0, sys;
...
extern void antmodel_s_gamp(int sat, const pcv_t* pcv, double nadir, double* dant)
{
	int i, sys;
